...
namespace Plib
{
	namespace Text
	{
		...
		protected:
			...
			INLINE bool __FlushData( const String & __data ) {
				// Parameters checking.
				if ( __Handle == -1 || __OStatue == FS_READ ) return false;
				
				if ( __data.Empty() ) return true;
		...
		public:
			...
			// Set the file path
			INLINE void FilePath( const String & _filePath ) {
				if ( !_filePath.RefNull() )
				//if ( _filePath != String::Null )
					__FilePath.DeepCopy( _filePath );
			}
			
			// C'Str, init the filestream with the filepath,
			// the filepath must not be Null.
			FileStream_( const String & _filePath ) : __Handle( -1 ) {
				assert( _filePath != String::Null );
				__FilePath.DeepCopy( _filePath );
			}
			...
			bool WriteLine( const String & _line ) {
				if ( !Write( _line ) ) return false;
				return Write( "\r\n" );
			}
			
			// Append the data to the file.
			bool Append( const String & _data ) {
			...
			bool AppendLine( const String & _line ) {
			...
			String Read( ) {
				if ( __Handle == -1 || __OStatue != FS_READ ) return String::Null;
				char _c;
				String _word;
				...
				// if error occurred..
				if ( _ret < 0 ) return String::Null;
				return _word;
			}
			
			// Read specified length data.
			String Read( Uint32 _length ) {
				if ( __Handle == -1 || __OStatue != FS_READ ) return String::Null;
				char _c;
				Int32 _ret;
				String _word;
		...
		public:
			...
			ReadStream_( const String & _filePath ) : TFather( true ) {
				CONSTRUCTURE;
			...
			// Read a word
			INLINE String Read( ) { return TFather::_Handle->_PHandle->Read(  ); }
			// Read specified length data.
			INLINE String Read( Uint32 _Length ) { return TFather::_Handle->_PHandle->Read( _Length ); }
			// Read a line.
			INLINE String ReadLine( ) { return TFather::_Handle->_PHandle->ReadLine( ); }
			// Read all data.
			INLINE String ReadToEnd( ) { return TFather::_Handle->_PHandle->ReadToEnd( ); }
		...
		public:
			// C'Str, set the file path and open for reading.
			WriteStream_( const String & _filePath ) : TFather( true ) {
				CONSTRUCTURE;
				TFather::_Handle->_PHandle->FilePath( _filePath );
			...
			// Write data.
			INLINE bool Write( const String & _data ) { 
				return TFather::_Handle->_PHandle->Write( _data );
			}
			// Write Line.
			INLINE bool WriteLine( const String & _data ) {
		...
		public:
			// C'Str, set the file path and open for reading.
			AppendStream_( const String & _filePath ) : TFather( true ) {
				CONSTRUCTURE;
				TFather::_Handle->_PHandle->FilePath( _filePath );
			...
			INLINE bool Save( ) { return TFather::_Handle->_PHandle->Save( ); }
			// Write data.
			INLINE bool Append( const String & _data ) { 
				return TFather::_Handle->_PHandle->Append( _data );
			}
			// Write Line.
			INLINE bool AppendLine( const String & _data ) {
		...
		struct File
		{
			...
			static bool IsExisted( const String & _filePath ) {
			#if __DEF_WIN32
			...
			static bool Create( const String & _filePath ) {
			...
			static bool Delete( const String & _filePath ) {
				return remove( _filePath.c_str() ) == 0;
			}
			
			// Move the file to other place.
			// if the file is not existed, return false.
			static bool Move( const String & _source, const String & _dest ) {
				return rename( _source.c_str(), _dest.c_str() ) == 0;
			}
			
			// Copy a file to another one.
			static bool Copy( const String & _source, const String & _dest, bool _overwrite ) {
				...
				FILE * pWrite = fopen( _dest.c_str(), "w" );
			...
			// with the flag.
			static bool Append( const String & _filePath, const String & _data, bool _onErrorPrint = true ) {
			...
			static bool Clear( const String & _filePath ) {
			...
			// Resize a file.
			static bool Truncate( const String & _filePath, Uint32 _size, bool _beHole = true ) {
			...
			// Get the file's info.
			// Create time
			static Uint32 CreateTime( const String & _filePath ) {
			#if _DEF_WIN32
				struct _stat st;
			...
			// Modify time
			static Uint32 ModifyTime( const String & _filePath ) {
			#if _DEF_WIN32
				struct _stat st;
				if ( 0 != _stat(_filePath.c_str(), &st) ) 
				{
			...
			// Last Access Time
			static Uint32 AccessTime( const String & _filePath ) {
			#if _DEF_WIN32
				struct _stat st;
				if ( 0 != _stat(_filePath.c_str(), &st) ) 
				{
			...
			// File Size
			static Uint32 FileSize( const String & _filePath ) {
			#if _DEF_WIN32
				struct _stat st;
				if ( 0 != _stat(_filePath.c_str(), &st) ) 
				{
